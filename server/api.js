...
module.exports = function (app, config) {
    ...
    Rsvp.findById(req.params.id, (err, rsvp) => {
      ...
      if (rsvp.userId !== req.user.sub) {
        return res.status(401).send({
          message: 'You cannot edit someone else\'s RSVP.'
        });
      }
  ...
  app.post('/api/event/new', jwtCheck, adminCheck, (req, res) => {
    Event.findOne({
      ...
      if (existingEvent) {
        return res.status(409).send({
          message: 'You have already created an event with this title, location, and start date/time.'
        });
      }
    ...
    Event.findById(req.params.id, (err, event) => {
      ...
      Rsvp.find({
        ...
        event.remove(err => {
          ...
          res.status(200).send({
            message: 'Event and RSVPs successfully deleted.'
          });
        });
      });
    });
